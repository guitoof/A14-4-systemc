...
SC_MODULE(cl_platform_prv) {   
  ...
  public:
    ...
    sc_signal<bool>     ready_outm_slv_port;
                      ...
                      sc_module(nm)
    {
      ...
      //wiring pic to OUTPUT MEM
      pic->request_slave[N_CORES+2 + N_HWS_PORTS ](request_outm_slv_port);
      ...
      pic->pinout_slave[N_CORES+2 + N_HWS_PORTS + N_OUTM_PORTS](pinout_sem[0]);

      //wiring pic to DMA slave itf
      pic->request_slave[N_CORES+2 + N_HWS_PORTS + N_OUTM_PORTS + 1](request_dma_slv);
      pic->ready_slave[N_CORES+2 + N_HWS_PORTS + N_OUTM_PORTS + 1](ready_dma_slv);
      ...
      outm->clock(ClockGen_1);
      outm->slave_port (pinout_outm_slv_port);
      outm->sl_rdy(ready_outm_slv_port);
      outm->sl_req(request_outm_slv_port);
...
//           sc_trace(tf, ARM11_IDLE[i], buffer);
        ...
        //OUTPUTMEM
        sprintf(buffer, "pinout_outm_slv_port");
        sc_trace(tf, pinout_outm_slv_port, buffer);
        sprintf(buffer, "request_outm_slv_port");
        sc_trace(tf, request_outm_slv_port, buffer);
        sprintf(buffer, "ready_outm_slv_port");
        sc_trace(tf, ready_outm_slv_port, buffer);
